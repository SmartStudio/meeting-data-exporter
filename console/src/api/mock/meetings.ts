...
export const MEETINGS: Meeting[] = [
  {
    ...
    /* 前三条**逐字相同且连续**，而且跨了分钟——这是演示世界里唯一一处折叠态
       （`display.groupHistory`：连续同句折成一行 + `×N` + 时间区间）。

       它在这里是为了让门禁**真的扫得到那一行**。a11y 的第 3 项（375px 无横向
       溢出）走的是演示世界；折叠行比普通行多一列 `×N`，演示世界里一条折叠行都
       没有的话，那一项永远扫的是两列的普通行——一个报告"通过"却没检查过折叠态
       的门禁，比没有这个门禁更糟（同 CSS 令牌那条的空扫描保护）。
       删掉这三条之前先想清楚这一点。 */
    history: [
      { at: ts(2026, 8, 23, 14, 4), text: '知识库索引器 取用了 AI 纪要' },
      { at: ts(2026, 8, 23, 14, 3), text: '知识库索引器 取用了 AI 纪要' },
