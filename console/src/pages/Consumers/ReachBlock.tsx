      ...
      {line.kind === 'none' && (
        <>
          ...
          {/* 已授权但取不到时"为什么"由 `BlockedCell` 那一列回答，这里不重复；
              ...
              这条出路是**两步**，不是二选一：spec §1.3 的三个条件是「与」，
              授权为零时 `computeProgramInventory` 按授权行枚举、直接返回空清单，
              而就算补上了授权，采集权限栈里没有一条准许的规则时兜底仍是 deny。
              原来这句写成「到自动规则里放行，或在会议记录里逐场授权」，
              照着它只做一步的人，看到的数永远是 0。

              **顺序是先规则、后授权，不能反过来**：会议记录页那一格
              （`Meetings/display.ts` 的 `grantCellKind`）只在规则栈已判「准许」时才画出
