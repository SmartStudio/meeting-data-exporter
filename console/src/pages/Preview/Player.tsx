 ...
 * ## 进度条上的标记是转写分段，不是章节
 *
 * 原型那句注释写的是「章节标记」。章节现在是有的——开了智能录制的录制文件，
 * 腾讯会给章节，走时间轴那个端点下发，时间轴 tab 单列一节渲染它。但**走时条上
 * 的竖线仍然只标转写分段**：分段密得多，也是「跳到某句话」的那个粒度；章节点
 * 在列表里点。所以 `aria-label` 与轨道的 `title` 照转写分段说。
 */
...
const TRACK_HINT = '竖线是转写分段的起点，不是章节——章节在下面的列表里'
