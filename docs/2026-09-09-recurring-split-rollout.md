...
- ~~**STS token 全局单例**——本机与服务器同时跑，后申请的那个会把先申请的打成「非法」，
  回调只到其中一台（见 memory `sts-single-active-token-per-app`）。~~ **STS 链路已于
  2026-09-10 整体移除，这条顾虑作废**——但两台调度器仍会抢同一批归档文件，第一条
  理由仍然成立，所以第 4 节起服务器的调度器**之前**，仍须先确认本机的已经停了。
...
起服务器调度器之前再确认一次本机的调度器已经停了（避免两台抢同一批归档文件；
STS token 全局单例那条顾虑已随 STS 链路于 2026-09-10 移除而作废）。
