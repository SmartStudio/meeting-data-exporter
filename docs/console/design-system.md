...
琥珀的两个含义都意味着「这需要你看一眼」，而一条 deny 规则命中是**规则系统在正确
地干活**——原型里的实例是「标题含面试/薪酬/绩效 → 禁止采集」，那是刻意配置的隐私
规则，绝大多数被拒的会议是**故意且永久**被拒的。把它画成琥珀，一个配了这类规则的
组织就会有一大片行永久琥珀，而真正该被看见的琥珀（有人绕过了规则、还剩三天到期）
会淹死在里面。一直响的警报等于没有警报。

「允许」与「拒绝」的区分由**明确的文字**承担（「规则禁止采集」vs「未归档」），
在有 `AllowState` 圆点的页面上再加一层圆点——**都不是理由文字着色的职责**。
...
`--rail-w` 是唯一一个**有条件第二值**的骨架令牌：`@media (max-width: 70em)` 下
...
它们**不借间距令牌**：md 恰好等于 `--s-8`(32) 但语义不是「一格间距」，
...
| 横向溢出 | 1440 / 1050 均无；**375 是已知缺口**（见 [`spec.md` §11 #2](spec.md)） | ⚠️ **原写「1440/1050/375 均无 ✅」是假的，两重意义上的假。** 其一，那次测量用的是 `documentElement.scrollWidth`，而本页面实测 `documentElement.scrollWidth = 375 = clientWidth` 却 `body.scrollWidth = 781`——裁剪链 `div.wrap{overflow-x:hidden}` → `body{overflow-x:clip}` 把它吃掉了，**那条断言在这个页面上不可能红**。成因是 `base.css` 那条**一次命中两个元素**的 `html, body { overflow-x: clip }`——隔离实验（375px `/meetings`，逐个放开、其余不动）：两个都 clip 时 `de=375`／放开 `html` 时 `de=781`／放开 `body` 时 `de=781`，**要两个都是 clip 才死，放开任意一个它就会红**；而 `Table` 的 `.wrap{hidden}`／`.scroll{auto}` 对这个数字**毫无贡献**（表格那 1020px 在 `.scroll` 这个真滚动容器就被吸收了，压根没往上冒，那是设计好的行为）。其二，375px 下实际有 8 类元素跑出视口够不着。**只量 `scrollWidth` 会给「被裁掉所以量不出来」发通行证**，必须同时验证元素在视口内可达 |
| 触控目标 | 输入类 ≥44px | ✅ |
