...
>
> **第二次订正（2026-09-02，视觉验收之后）**
> 上面那条订正只改了它当时查到的两行。**「触控目标」那一行是第三条假保证**——
> 它写着 `✅`，而门槛七项检查里没有任何一项在量触控尺寸。查出来时实际违反的有：
> 审计与会议记录两个「每页条数」下拉 51×20 / 44×20（四个视口全违反）、
> 三个原生 checkbox 13×13、登录页「记住我」的 label 只有 20.1px 高。
> 现在由**第 8 项**守。同一个教训第三次出现，值得记下它的形状：
> **一张表里只要还有一行的「谁在守」是空的，那一行迟早会变成假的。**
...
| 触控目标 | 输入类有效热区 ≥44×44 | **第 8 项**。量的是**有效热区**不是元素自己的盒子：原生 checkbox 恒为 13×13，把方框画大会让它看起来像另一种控件，正确做法是让包着它的 `<label>` 当热区（点 label 就是点它，原生行为），所以扫描会往上取最近的 label 祖先、两个盒子取更大的那个。只管输入类，与本行措辞一致：button / a 由 [`spec.md` §10](spec.md)「移动端只保证能看、不保证能改」那条界线分别处置，混在一起会逼着把密集后台里的写操作按钮也撑到 44。顶栏那个原型专用的「系统状态」下拉按**结构**豁免（在 `protoGroup` 子树里），不写成类名名单——名单会在改名时静默失效 |
