...
1. 打开控制台「定时任务」页，找到原因是「录像：腾讯那边没有这个文件」的那一组
   （屏幕上写着「23 场会议」这种数字——33 是资产行数，表上归并到会议，两个数不一样）
2. 点这一组的 **「全部重试」**
...
**但控制台产物要跟着一起回滚。** `console/dist` 不在 git 里、是逐请求从盘上读的（`src/http/static.ts`），代码回滚不会动它——而新产物**要求** `failures[].detail`（第 1 节那条顺序警告的反面）。回滚网关之后必须在旧代码上重新 `cd console && bun run build`，否则「定时任务」这一页会整页报 `ApiShapeError`。顺序与上线相反：先回滚代码起网关，再 build 控制台。
