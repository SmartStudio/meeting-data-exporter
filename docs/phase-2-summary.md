...
**已被真实验证**（Stage 8/9，2026-08-26 补跑；判据与数字见
[`m3.5-stage8-9-plan.md`](m3.5-stage8-9-plan.md) §0.1，本文不重复）：完整闭环（3 场会议
21 个资产，`completed=21 failed=0`）· 幂等（第二遍 `completed=0`）· 断点续传（打断在
7,454,720 字节，续传**从打断点继续而不是从 0**）· 崩溃恢复（租约过期后被重新领取并完成）。
**其中最值钱的一条是「对象存储支持 Range」有了真实证据**——`HTTP 206` +
`Content-Range: bytes 0-1023/205818547`，断点续传不再是建立在推断上的机制。
顺带实测：下载地址 TTL = **21600 秒**（6 小时）。

**仍未被真实验证**（**剩下的唯一一项**）：AI 纪要延迟产出的探测。它卡在数据不在代码——
要一场**刚结束、AI 纪要还没生成**的会议才触发得了，手上最新的会议是 08-20，纪要早就齐了，
`asset_probes` 一条都没有。下次有会议刚结束时立刻跑。

**顺带订正原文一句**：「服务端的归档 worker 现在就在持续跑它们」**不成立**。
生产服务端只部署了网关（`/home/ubuntu/mde/app`，无 `packages/`、无 worker 进程、无 crontab，
`migrations/` 只有 001 而库里的表到 003），归档链路在生产上从没跑起来过。
上面四条通过的机制**都是在客户端引擎里验的**，归档 worker 那个宿主至今零运行证据。
