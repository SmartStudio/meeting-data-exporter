...
**`client/` 至今只对「我们自己写的假网关」验证过。** 假网关是照着 `gateway/client.ts` 的期望
写的，两者必然吻合——**这不构成对真实网关的证据**。spec §17 明确列出的「已知未知」仍待核实：

- `listAssets` 响应里 `remote_id` / `state` / `allow_download` / `file_type` / `bytes_expected`
  的确切字段名（目前按网关 §5.2 契约推断）
- 加上 M2 遗留的 `preflight --sample-user` 实测与 deploy.md §11 的 webhook 线路格式核实

接缝已留好：字段映射集中在 `gateway/client.ts` 的一个函数里，真实字段名不同只改那一处；
`assetId` 全程当不透明串传递、不解析格式，天然免疫。详见 roadmap 的 M3.5 章节。

## 8. 下一步

1. **M3.5 真实环境联调** —— 当前下一步，也是投产的实际门槛（见 `docs/roadmap.md`）
2. M4（存储扩展 OSS / NAS）与 M5（桌面应用）—— 联调通过后可并行
