...
| # | 任务 | 落点 | 依赖 |
| --- | --- | --- | --- |
| **T6** | `meeting_grants` / `meeting_overrides` 表与 store | `migrations/005` · `src/store/grants.ts` | T1 |
| **T7** | 人工改写覆盖层 | `src/policy/override.ts`，在引擎**之外** | T3 · T6 |

> **落点更正**：T6 的迁移编号是 **005**，不是原计划写的 004——004 被 T1 占用了。

**T7 必测**：改写优先于任何规则（包括 priority 最高的那条）；改写只对该场会议生效；
撤销改写后回落到规则判定。

**T6 落地时的四处裁定**（`afdc9d1`，照 §3.4 的体例）：

| # | 事 | 裁定 |
| --- | --- | --- |
| **D-l** | 撤销授权：软删除还是删行 | **软删除**，写 `revoked_at`。spec §4.10 要求审计能回答「什么时候授权给谁、什么时候撤的」，删行就答不了。重新授权 = **插入新行**，不是把旧行的 `revoked_at` 清零——历史是一串事件，不是一个可翻转的开关 |
| **D-m** | 「同时只能有一条生效授权」由谁保证 | **由数据库**：`revoked_at BIGINT NOT NULL DEFAULT 0`（0 = 未撤销）配 `UNIQUE KEY (meeting_id, sub_meeting_id, program_id, revoked_at)`。生效行的 `revoked_at` 恒为 0，于是同一 (会议, 场次, 程序) 只能有一条生效行。这是授权中枢，DB 强制的不变量胜过应用层约定 |
| **D-n** | `asset_types` 的取值域 | **三态**：`NULL` = 不额外限制（以规则栈判定为准）· 非空数组 = 白名单 · `[]` = **什么都不授权**。不许把空数组解释成「不限制」——在授权中枢里让空集合意外等价于全集，正是 §6「不许静默放行」要防的事故 |
| **D-o** | `meeting_overrides.kind` 的取值域 | 与 `StackKind` **逐字一致**（`fetch`/`archive`/`allow`）。`effect` 存 `VARCHAR(255)`，因为归档栈的 effect 是目录模板不是枚举。一场会议的每个 kind 各自独立改写 |

**D-m 的一处更正（落地时才发现，原裁定说错了）**：我原本写的是「重复撤销要幂等」，
把两种情况混成了一种：

- 撤销一条**已撤销**的授权——`WHERE revoked_at = 0` 一行都匹配不上，天然是无操作，
  **根本走不到唯一键**
- 授权→撤销(t)→重新授权→**再撤销(同一毫秒)**——这才是真会撞唯一键的那种

照原裁定的字面实现（撞了就当无事发生），第二种情况下**那条管理员明确要求撤销的授权会继续生效**。
实现改成：撞键就把 `revoked_at` 让开一格重试，让满 64 格仍失败就抛出去。
**撤销时刻差 1ms 无伤大雅，丢掉一次撤销才有伤。**

同源的一个入口坑：`now = 0` 会写出一行看起来仍然生效的记录（0 正是「未撤销」哨兵），
所以 `now <= 0` 直接抛错。

**T6 在裁定之外自加的两列**（均已复核）：

- `meeting_overrides.asset_types`（同 D-n 的三态）。没有它，一条 `allow` 改写只能表示
  「八类全放行」，而改写**优先于所有规则**——等于一次人工改写把规则原本限定的资产范围
  悄悄放宽，是 D-n 想防的那类事故换了个入口
- `meeting_overrides.reason`。§1.3 要求界面随时能回答「为什么取不到」，规则那边这句话来自
  `policy_rules.note`；改写优先于规则却没有等价的一句话，被改写的会议在详情抽屉里
  就只剩一个没有出处的结论

**没有**加 `granted_by` / `revoked_by`：操作者归 `audit_log` 管（`src/store/audit.ts`
已有 `actor_type`/`actor_id`），复制进来会开第二个来源。两张表都**不建 FK** 到
`service_accounts`，与仓库既有表一致——一个程序被删掉不该顺带抹掉「它曾经被授权过哪些会议」。

**T7 落地时的五处裁定**（`8a62bb4`）：

| # | 事 | 裁定 |
| --- | --- | --- |
| **D-p** | `DecisionSource` 给改写加几个取值 | **两个**：`override` 与 `override_invalid`。改写的 effect 来自管理员在界面上填的自由文本，比 `policy_rules.effect` 更容易脏；少一个就等于把「管理员填错了」和「管理员就是这么定的」混成同一件事，详情抽屉再也分不开 |
| **D-q** | 被改写掉的那个判定要不要留 | **留**，`OverriddenDecision.overriddenFrom`。§1.3 要求界面随时能回答「为什么取不到」——只剩一个结论的话，管理员看不到「规则本来会放行、是人工关掉的」，而这恰恰是他最需要知道的。`trace` 照抄规则侧那份：规则确实被考察过，只是最后没轮到它说话 |
| **D-r** | 改写的 `assetTypes` 为 `null` 时取什么 | **沿用 `overriddenFrom.assetTypes`**。后果是「改写成 allow 但没指定范围」在规则侧本来就判 deny 时，结果是**放行了但一类都取不到**——这是故意的安全侧，必须在 `issues` 里说人话。**不许**把 `null` 解释成「全部八类」：那是一次没填完的改写把规则原本限定的范围悄悄放宽 |
| **D-s** | 要不要在 `override.ts` 里另写一份 effect / 资产名校验 | **不许**。把 `normalizeEffect` / `normalizeAssetTypes` 从 `stacks.ts` export 出去复用。两份规范化逻辑迟早漂移，漂移的后果是同一类资产在规则路径和改写路径下待遇不同（D-c 记着这个项目已经为资产别名吃过一次亏） |
| **D-t** | 改写的 `reason` 放判定的哪个字段 | **`note`**，`ruleId` 置 `null`。`note` 的语义是「决定这次判定的那个东西身上的、人写的说明」，规则与改写在这一点上同构 |

**T7 落地时发现 D-p 修错了一层——`kind` 比 `effect` 更脏，而且没有安全侧可落：**

| # | 事 | 裁定 |
| --- | --- | --- |
| **D-u** | `meeting_overrides.kind` 的取值域由谁保证 | **写入侧拦死，两道防线。** 求值层拦不住也报不出来 |

两种情况，都不是理论问题：

- **kind 填成另一栈**：一条 `fetch` 改写（`effect: 'all'`）被套到归档栈上时，
  `normalizeEffect('archive', 'all')` 认为 `all` 是一段**合法的非空目录模板**，
  于是这场会议的录像会被归档进一个叫 `all` 的目录。D-s 那条「复用规范化函数」**拦不住这个**
  ——被复用的函数无从知道这一行原本是为哪一栈写的
- **kind 填成三栈之外**（`'fecth'`）：没有任何一栈认领它，`indexOverrides` 只能丢掉，
  于是管理员明确做出的一个决定变成一次**界面上毫无痕迹的空操作**。正是 §6「不许静默」
  要防的那类错误，只是换了一层

落地：`src/store/grants.ts` 的 `putOverride` / `revokeOverride` 做运行时校验
（TypeScript 管得住我们自己的调用点，管不住 HTTP 请求体反序列化出来的值），
`migrations/006` 加一条 CHECK 管住绕开 store 的直接 SQL。**授权中枢两道都要。**

> **编号说明**：D-a～D-e 是 T2/T3 期的裁定，D-f～D-h 属 T5，D-i～D-k 属 T4，
> D-l～D-o 属 T6，D-p～D-u 属 T7 与本次修补。**T8 / T9 起改用任务内编号**
> （`D-T8-a`、`D-T9-a`…）——单条字母序列已经撞过一次车，并行落地下还会再撞。
...
**T8 落地时的裁定**（`a125169`，起用任务内编号，理由见上面的编号说明）：

| # | 事 | 裁定 |
| --- | --- | --- |
| **D-T8-a** | 「在保留期内」的判据 | **`localPurgedAt === null`（文件真的还在）**，不是 `expiresAt >= now`。spec §1.3 括号里写的是「本地文件还没被删」，那才是定义。由此产生一个真实状态：**窗口已过期但清理被暂停**的会议文件还在，所以仍然可取——清单描述的是此刻的事实，不是政策 |
| **D-T8-b** | 「N 天内到期」谁来判 | 这一层只给 `expiresAt` 这个事实，阈值在调用方。阈值写死在 worker 里的话，界面上想换个天数就得改 worker |
| **D-T8-c** | 三个「与」缺一个时怎么答 | **三个都报，不是报第一个就停**。只报第一个的话，管理员补上授权后发现还是取不到，来回三趟。理由分三种（去授权页 / 去 NAS 取 / 去规则页），不许合并成一句「不满足条件」 |
| **D-T8-d** | 会议在 `meetings` 表里查不到 | 落到拒绝一侧，`decision` 给 `null` 而**不是编一个从未发生过的判定**。详情抽屉显示一条假判定比显示 null 更难查 |

**D-T8-e 是对我原裁定的更正**：我原本写的是「资产类型 = 规则 ∩ 授权 ∩ 改写」三者求交。
**这条会废掉「改写优先于所有规则」。** `applyOverride` 是**替换**语义（改写指定范围就用
改写的，`null` 才沿用规则那份），因为改写优先。照原裁定再与规则侧交一次，一条把 deny
翻成 allow 的改写就会与「规则侧的空集」相交、算出空集——**改写等于没写**。
正确的是：`decision.assetTypes` 已经是改写生效后的那一份，**只再交授权行**。

**T9 落地时的裁定**（`6370117` + `27a7ccd`）：

| # | 事 | 裁定 |
| --- | --- | --- |
| **D-T9-a** | 判 `skip` 后要不要留痕 | **要**，每一场都带理由 `console.warn`。一场会议悄悄没被归档是这个系统里最难排查的一类现象：库里没记录、NAS 上没目录、日志里没有一行 |
| **D-T9-b** | 模板渲染不出合法路径时 | **落到不归档 + 明确理由**，不要「尽力而为地拼一个」。把录像写到管理员没想到的地方，比不写更糟。未知占位符**判失败**：原样保留会产出字面带 `{年份}` 的目录，看起来像成功了 |
| **D-T9-c** | `{年}/{月}` 取哪个时刻 | **会议 startTime**，与本地归档区同源。原实现用的是**归档时刻**——七月的会议八月归档会落进 `2026/08/`，与本地错开一个月。改它是安全的：`nas_path`/`nas_dir` 都存在库里，`retention.ts` 读记录而不是重算 |
| **D-T9-d** | 目录模板是绝对路径还是相对 | **相对 `MDE_NAS_ROOT`**，绝对路径与 `..` 一律判不归档（`writeNasSidecars` 用 `relative(nasRoot, nasDir)` 算 sidecar 落点，跑出 nasRoot 会把两个 JSON 写到挂载点外）。**代价**：原型的种子规则与新建规则的默认 effect 原本全是 `/nas/...` 绝对形式，照着填一条得到的是「一场都没归档」，已一并改成相对形式 |
| **D-T9-e** | 元数据取不到时归不归档 | **不归档**。归档规则的条件与 `{年}/{月}/{标题}` 都要会议字段，没有字段就是判不出来。与 US-6.2「元数据取不到不算失败」相冲，取前者：不归档是可恢复的（文件留在本地、每轮喊一声），归到猜出来的位置不可恢复（文件散在 NAS 上，而到期清理会把本地副本删掉） |

**`skipped` 拆成两个数**（D-T9-a 的落地补充）：`skipped` 是**规则就是这么定的**
（命中 skip、走兜底，正常运转），`undecidable` 是**判不出来**（模板写坏了、元数据取不到）。
合在一个数字里的话，一条写坏的规则在轮末汇总里与「今天没有会议需要归档」长得一模一样。
`undecidable` 仍然不进退出码——让它进的话，一场永远匹配不上的会议会把退出码永久钉成非零，
那种警报很快就没人看了。

---

## 4.5 覆盖层的接线（2026-08-26 补，本阶段真正的收尾）

T7 交付的是覆盖层**本身**。但「人工改写优先于所有规则」（spec §5.4）要成立，
它必须**接到每一条判定路径上**：

| 判定路径 | 状态 | 说明 |
| --- | --- | --- |
| 网关采集权限（`policy/access.ts`） | ✅ `d075c19` | **T4 落地时覆盖层还不存在，曾经完全绕过改写**——管理员按下的「这场不许取」拦不住 `download-url`。这是本阶段最大的一个缺口 |
| 归档目录（`worker/archive.ts`） | ✅ `27a7ccd` | T9 标好的接缝，`override` 做成必填参数 |
| 采集清单（`worker/visibility.ts`） | ✅ `a125169` | T8 自带 |
| 影响预览（`policy/preview.ts`） | ✅ `969e741` | T5 的 `shielded` + T7 的 `overriddenPreviewFilter` |

两处接线各自的取舍：

- **网关这边改写由 `AccessGate` 自己取**，不像 `archived` 那样让调用方传。
  这是数据出境的闸门（spec §1.4），让调用方负责递等于每新增一个调用点就多一次
  「忘了递就静默绕过」的机会，而绕过的方向是**放行**。漏了 `archived` 最多让一条
  `arch` 规则判错，漏了改写是让管理员明确按下的那个「不许取」失效
- **归档这边 `override` 是必填参数**，与 `rules` 一样。可选参数漏传就是静默按规则判，
  而「这场归到别处」不生效和没按一样，没有任何痕迹。改成必填后编译器把每个调用点
  都逼了出来
- **两处都要按 `kind` 筛**：一条 allow 改写的 effect `allow` 会被
  `normalizeEffect('archive', …)` 认成一段合法的非空目录模板
