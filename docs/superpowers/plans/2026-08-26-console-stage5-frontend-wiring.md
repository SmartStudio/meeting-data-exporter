...
- **跨栈契约测试，这个仓库还没有**（F9 接完线之后剩下的那个洞）。
  前端测规则编辑器用的是 `tests/helpers/rulesSchema.ts`——一份**从后端生成的
  静态夹具**。生产代码一个取值都不存（真读 `GET /rules/schema`），但**后端加一个
  运算符时这份夹具不会变，前端测试会继续绿**。

  所以前端测试现在只能证明"忠实渲染收到的东西"，不能再证明"前后端一致"。
  守住一致性的只剩后端那侧：`console-rules-schema.test.ts`，加上 A9 给动作名
  加的类型收窄与源码扫描两道门。**够用，但不是同一件事** ——
  真正的跨栈契约测试要么让夹具从后端源头生成并在 CI 里比对，
  要么起一个真网关跑几条端到端。两条路都不小，所以记在这里而不是顺手做。
