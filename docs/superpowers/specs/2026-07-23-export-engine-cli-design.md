...
### 17.1 M3.5 联调回填（2026-08-25 补记，联调发生在 2026-08-21～08-22；2026-08-26 补跑 Stage 8/9 后再次回填）

只回填**有提交或实测为证**的部分。没跑到的仍标未知——把推断写成结论正是本节存在的理由。
2026-08-26 那轮的完整记录在 [`docs/m3.5-stage8-9-plan.md`](../../m3.5-stage8-9-plan.md) §0.1。

| # | 状态 | 结论 |
| --- | --- | --- |
| 1 | ⬜ **仍未知** | AI 待产的真实表现要靠 runbook Stage 9.4（找一场刚结束的会议跑 `run`）验证。2026-08-26 那轮**尝试过但触发不了**：手上最新的会议是 08-20，纪要早就齐了，`asset_probes` 一条都没有。**这一条现在只差一场刚结束的会议**——不缺代码、不缺工具、不缺环境。§9 判定表对两种表现都安全，架构不受影响 |
...
| 4 | 🟡 **验了一半** | **TTL 已知**：下载地址 TTL 2026-08-26 实测 = **21600 秒**（6 小时），比本文与 runbook 原先假设的「有效期很短」宽松得多。**401 响应形状仍未知**：没有任何实测记录，客户端「从 401 稳定识别令牌过期」这一步至今没有真实证据。两半必须分开读，别因为上半绿了就当整条已知 |
...
### 本节最重要的一条：§10 的断点续传成立了

~~§10 的断点续传全靠对象存储支持 Range，那一条至今没有真实证据。~~
→ **2026-08-26 实测：`HTTP 206 Partial Content` + `Content-Range: bytes 0-1023/205818547`。
对象存储支持 Range，本文 §10 描述的续传机制是真的。** 它不再是建立在推断上的设计。

端到端也跟着验了一遍：对一个 205,818,547 字节的录像 Ctrl-C 打断在 **7,454,720 字节**，
重跑 `execute` 后续传采样 `7.45MB → 19.2MB → 30.4MB → 41.8MB → 53.0MB`——**起点正是打断点，
不是 0**，最终字节数与 `Content-Range` 报的总长一致。这是「Range 头被真的用上了」的证据，
而不只是「服务端愿意返 206」。

**Stage 8/9 的其余结论**（完整记录见 [`m3.5-stage8-9-plan.md`](../../m3.5-stage8-9-plan.md) §0.1，
本文不重复细节）：完整闭环 ✅（3 场会议 21 个资产，`completed=21 failed=0`，2.5 分钟）·
幂等 ✅（第二遍 `completed=0`）· 崩溃恢复 ✅（租约过期后被重新领取并完成）。

**剩下的投产门槛只有一项**：Stage 9.4 AI 纪要延迟探测（= 上表第 1 条），加上上表第 4 条
未验的那一半（401 响应形状）。

**另有两处本文的推断被实测推翻，值得单独记**：

- **`bytes_expected` 对这批资产根本不返回**，`_manifest.json` 的 `bytes` 字段因此恒为 null。
  上表第 3 条说「文本类可能缺 `bytes_expected`」——实际比这更彻底，视频也没有。
  §10 里「下完即完成 / 靠 hash 兜底」的退化路径不是边角情况，**它就是常态路径**。
- **目录名里的时间是 UTC**：08-12 16:26（本地）的会议落进 `2026-08-12_0826_…`。
  不是错，但按目录找会议的人会对不上。
