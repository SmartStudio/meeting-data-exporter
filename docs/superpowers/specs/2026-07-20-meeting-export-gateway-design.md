...
3. **重试必须重新签名**：时间戳与 nonce 均需重新生成，否则触发 `190301`。
...
1. **事件加解密算法细节**：按[事件加解密文档](https://cloud.tencent.com/document/product/1095/54658)实现 EncodingAESKey 的解密流程。
2. **企管后台配置**：需管理员在腾讯会议企管后台完成——创建企业自建应用取得 AppId / SdkId / SecretId / SecretKey；配置事件订阅 URL（支持四级域名）、Token、EncodingAESKey；勾选公共事件「STS Token 生成」；为 operator 账号授予「管理企业录制」「查看企业录制」权限。
3. **operator_id 归属**：所有接口须传 `operator_id` + `operator_id_type`，且该账号需为超级管理员/管理员或具备企业录制管理权限。需确定网关使用哪个账号作为固定 operator。
4. **企微应用**：需创建企业微信自建应用用于扫码登录，取得 CorpId / AgentId / Secret。
