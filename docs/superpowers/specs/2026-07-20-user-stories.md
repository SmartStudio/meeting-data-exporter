...
- 系统内至少保留一个 **`admin` 角色**的账号：删除它、或把它降级成 `readonly`，两条路都拒绝，避免把自己锁在外面
  （**这一条原先写的是「至少保留一个账号」，那是错的**：1 admin + 1 readonly 时账号数是 2，
  admin 删掉自己会被放行，此后没人能写、没人能建号，bootstrap 又因表非空拒跑）
...
| US-3.5 管理运维账号 | ◐ | 后端四条端点齐全（建 / 删 / 列 / **改角色**）· 级联撤销会话 · 只读角色端到端成立（迁移 009 + 白名单式守卫 `src/http/middleware.ts:128-148` + 遍历路由表的回归网 `tests/http/console-readonly.test.ts` 19 挡 / 3 放 + 键盘绕过已修）· **「不能把自己锁在外面」两条路径都拦得住**：删除与降级共用同一个判据 `AdminStore.isLastAdminAccount`（守卫 `handlers/console/auth.ts` 的 `refuseIfLastAdmin`，死局用例 `tests/http/console-last-admin.test.ts`） | **阶段 5 没有做出账号管理界面**：`console/src` 对 `/admin/accounts` 的调用数是 **0**，改角色目前只能走接口（403 那句提示语已改成说这件事）。且 `docs/console/spec.md` 的页面清单里从来就没有这一页——不是排期没排到，是规格里就没有。（**2026-08-27 结清的两个洞**：① 角色只能建号时定、`src/store/admin.ts` 无 `updateRole` → 已加 `updateRole` + `PATCH /api/v1/admin/accounts/:id` + 审计动作 `change_admin_role`；② 「至少保留一个账号」数的是 `COUNT(*)` 不分角色 → 已换成「至少保留一个 admin」，且**降级与删除同一个判据**，因为降级是通往同一个死局的第二条路） |
...
| 2026-08-27（US-3.5 两个洞结清） | **改掉一条写错的验收标准**：「系统内至少保留一个账号」→「至少保留一个 `admin` 角色的账号，且删除与降级两条路都拦」。原文那句字面照做出来的守卫（`COUNT(*) <= 1`）正好放过它要防的那件事：1 admin + 1 readonly 时 admin 删得掉自己，删完没人能写、没人能建号、bootstrap 因表非空拒跑。连带补上改角色这条从来没有过的路径（`AdminStore.updateRole` + `PATCH /api/v1/admin/accounts/:id` + 审计动作 `change_admin_role`）——它同时是通往同一个死局的第二条路，所以与删除共用**同一个判据** `isLastAdminAccount`。§9.4 的 US-3.5 一行随之改写（仍是 ◐：账号管理界面还是没有） |
