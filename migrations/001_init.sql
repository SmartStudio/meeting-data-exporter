...
-- 不含 meeting_id / host_user_id / start_time 等策略判定所需字段，而会议列表接口不支持
...
-- 写入时机：每次从腾讯拿到会议列表时整批 upsert（src/tencent/records.ts）；MySQL 共享存储，多实例安全。
-- 2026-08-27 之后它还是精确查询（按会议号/会议 ID 点名查）的第一级，见 010 与 src/store/meetings.ts。
