...
test('03 断点续传：中断的 .part 在重跑后续传完成，字节内容完整一致', async () => {
  ...
  try {
    ...
    backend.setAssets(meeting.meeting_id, [{ asset_id: 'a-resume', asset_type: 'video', remote_id: 'rf-resume', allow_download: true, file_type: 'mp4', bytes_expected: content.length }])
...
test('04 链接过期：首链 403 → 客户端换新链续传，最终完整下载', async () => {
  ...
  try {
    ...
    backend.setAssets(meeting.meeting_id, [{ asset_id: 'a-expire', asset_type: 'video', remote_id: 'rf-expire', allow_download: true, file_type: 'mp4', bytes_expected: content.length }])
...
test('05 416：本地 .part 大于远端内容 → 丢弃重下，最终字节数正确', async () => {
  ...
  try {
    ...
    backend.setAssets(meeting.meeting_id, [{ asset_id: 'a-416', asset_type: 'video', remote_id: 'rf-416', allow_download: true, file_type: 'mp4', bytes_expected: content.length }])
...
test('06 崩溃恢复：running 且租约已过期的行被下一次 execute 重新领取并完成', async () => {
  ...
  try {
    ...
    backend.setAssets(meeting.meeting_id, [{ asset_id: 'a-crash', asset_type: 'video', remote_id: 'rf-crash', allow_download: true, file_type: 'mp4', bytes_expected: content.length }])
...
test('10 令牌过期：业务调用中途 401 → 网关客户端透明续期，流程仍完整完成', async () => {
  ...
  try {
    ...
    backend.setAssets(meeting.meeting_id, [{ asset_id: 'a-token', asset_type: 'video', remote_id: 'rf-token', allow_download: true, file_type: 'mp4', bytes_expected: content.length }])
