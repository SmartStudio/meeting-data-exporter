...
test('就绪资产建 pending 任务；不在清单的想要类型建 probing 探测', async () => {
  ...
  // 想要 video + ai_minutes：video 就绪→任务；ai_minutes 不在清单→探测
  const r = await discover({ gw, store }, { kind: 'range', from: 1, to: 2 }, ['video', 'ai_minutes'], 1000)
  expect(r.meetings).toBe(1)
  expect((await store.counts()).pending).toBe(1)                 // 只有 video 建了任务
  expect((await store.dueProbes(1000)).length).toBe(1)           // ai_minutes 在探测
})

test('allow_download=false 的想要资产直接 skipped，不建任务不留探测', async () => {
  const store = createStore(openDb(':memory:'))
  const gw = fakeGw({ m1: [
    { assetId: 'm1:rf1:ai_minutes:0', assetType: 'ai_minutes', remoteId: 'rf1', state: 3, allowDownload: false },
  ] })
  await discover({ gw, store }, { kind: 'range', from: 1, to: 2 }, ['ai_minutes'], 1000)
