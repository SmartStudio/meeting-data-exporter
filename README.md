...
> 企业版把会议开在腾讯云上，不等于会议资产在企业手里。官方给 AI 的入口（CLI / Skill / MCP）默认不发给企业账号。

![会议记录页：全企业录制的拉取、归档、授权状态一屏可见](docs/readme/01-meetings.webp)
...
腾讯会议企业版的问题分两类。第一类是资产权：录制和转写留在腾讯侧、挂在员工个人账号下，到期即删，企业没有自动落盘的官方能力。第二类是 AI 可达性：官方的 CLI、Skill、MCP 面向个人账号，企业账号申请灰度也只拿到个人 OAuth 范围。下面按产品事实逐条列出。

### 一、归档与资产权：数据不在企业自己的存储里

| # | 痛点 | 产品事实 | 对企业意味着什么 |
| --- | --- | --- | --- |
| 1 | 没有定期、自动落到企业存储的官方能力 | 企管后台只能人工勾选、异步打包、下载中心取件，文件会过期。没有按天或按周扫全企业并写入企业网盘或对象存储的功能。想自动化只能自建应用拿 AK/SK 自己落盘，下载地址会过期，账户级列表接口约每分钟 10 次限流 | 归档是人的任务，不是系统的任务。漏一周就漏一周 |
| 2 | 留存期限不统一，过期即删 | 个人中心的已结束会议约 30 天，会后文档约 30 天，聊天只在本机缓存、不漫游，企业账号到期后云录制约再留 90 天。额外存储是加购项。"无限云存储" 指空间额度，不是永久可导出 | 没有一个统一的保留期可以写进制度 |
| 3 | 资产挂在创建者名下，不挂在企业主体 | 录制、转写、纪要默认归会议预定者。主持人不是创建者时多数只能看。员工用个人账号开会，企业后台看不见。离职要手动"继承"，漏配即随账号消失 | 人走，会议资产跟着走 |
| 4 | 可导字段撑不起考勤与合规 | 创建者端名单只有昵称、首次进入、最后退出、次数、时长、身份。UserID、手机号、邮箱只在企管自定义导出里。显示名可改，同一人反复进出不合并 | 名单不能直接当考勤或合规凭证 |
| 5 | 聊天几乎不是企业数据 | 默认没有聊天导出，Webinar 聊天官方标注暂不支持导出。全文需要开聊天存档并走管理员 API，且只覆盖公共聊天 | 会中的决定和链接留在个人电脑上 |
| 6 | 转写不是事后能补的完整资产 | 会中没开转写就没有文字。方言、纯英文有限制，鸿蒙导出弱，移动端不能改逐字稿 | 转写缺失时，任何归档和 AI 流水线都是空的 |

没有进企业存储的资产仍在腾讯侧、在创建者账号下。企业只有登录查看的使用权，没有可迁移、可审计、可离线使用的副本。

### 二、对 AI 不友好：官方入口不面向企业账号

| # | 痛点 | 产品事实 | 对企业意味着什么 |
| --- | --- | --- | --- |
| 7 | CLI / Skill / MCP 默认不接企业版 | 个人版、专业版已开放。商业版、企业版需要提交灰度申请等待审批。Skill 专区取 Token 必须用个人账号，企业账号提示不在灰度范围内 | 企业买了企业版，AI 入口反而要用个人号 |
| 8 | 灰度批了也不是企业数据面 | CLI 和 MCP 用 OAuth 用户身份，只能碰该用户有权看的会议。不能作为企业服务账号扫全公司录制与纪要。超级管理员 Token 不等于全量归档接口 | 每个人一把钥匙，没有一把是企业的 |
| 9 | 开放的是会后只读片段 | 能做：约会、改会、取消、录制列表、下载地址、转写、关键词、智能纪要。不能做：会中实时转写流、把会中 AI 助手对接到自己的模型、默认把聊天喂给 Agent | 企业 Agent 只能在会后看片段 |
| 10 | 腾讯纪要不等于企业自己的模型产出 | 智能纪要接口返回腾讯侧摘要。按内部模板抽待办、对接 CRM、生成合规文档，必须把转写拉回来用自己的模型处理。企业版被 #7 挡住，只剩 REST API 一条路 | 想用自己的模型，就必须自己把字拿回来 |
| 11 | 权限模型让 Agent 看得见会、拿不到字 | 创建者没开允许下载、没开转写、分享范围受限、Token 不是创建者，接口直接返回无权限 | Agent 的可用范围取决于每个创建者的勾选 |

### 两条结论

| 结论 | 支撑机制 |
| --- | --- |
| 不能定期自动归档。没落到企业存储的，就不是企业的资产 | 无官方定时全量归档；留存期限分叉；资产在创建者账号；下载中心是人工任务；聊天与文档默认不进企业库 |
...
CLI 和 Skill 只能作为个人助手的补充。在走完这条路径之前，腾讯会议企业版提供的是在线协作权，不是可带走的数据所有权，也不是可被任意 Agent 调用的企业接口。
...
**访问控制。** 一个程序能取到一场会议的资产，要同时满足三个条件：这场会议授权给了这个程序，本地文件仍在保留期内，采集权限规则判定为允许。三个条件由不同页面分别维护。采集权限规则是数据离开企业边界的唯一出口。用一条高优先级的拒绝规则，就能让绩效、薪资、期权、组织调整这类 HR 会议照常拉取归档，但任何程序都取不走。

**凭证隔离。** 腾讯 AK/SK 只存在于网关和调度器进程。采集程序用 service account 向网关认证，网关完成鉴权、规则判定与审计后返回临时下载地址。终端上没有能删全公司录制的凭证。
...
| 组件 | 位置 | 职责 |
| --- | --- | --- |
| 网关 `gateway` | `src/` | 持有腾讯 AK/SK。JWT 鉴权、企微设备授权登录、身份映射、采集权限判定、签发临时下载地址、写审计。托管控制台静态页。可多实例 |
...
| mde CLI | `client/` + `packages/engine` | 采集程序的参考实现。通过网关发现会议并下载资产到本地，支持断点续传与崩溃恢复 |
...
- 调度器只能运行一个实例。网关可横向扩展。
- 网关签发的临时下载地址指向腾讯云端文件，不读取本地目录。"在保留期内"这一条件目前只在控制台的采集清单中判定，网关侧未强制。本地已清理但腾讯侧仍存在的会议，程序仍可获取。详见 `docs/console/spec.md` §11 第 10 行。
