...
test('同一字段的数组含多项时各自成为独立资产', () => {
  ...
  const assets = extractAssets('m1', '', 'rec-1', multi, true).filter((a) => a.assetType === 'ai_minutes')
...
test('allowDownload=false 时 ai_* 资产被标记不可下载', () => {
  const assets = extractAssets('m1', '', 'rec-1', detail, false)
...
test('字段缺失时不产生该资产（而非产生空资产）', () => {
  ...
  const assets = extractAssets('m1', '', 'rec-1', partial, true)
...
test('空数组字段不产生资产', () => {
  ...
  expect(extractAssets('m1', '', 'rec-1', empty, true).some((a) => a.assetType === 'ai_minutes')).toBe(false)
})
