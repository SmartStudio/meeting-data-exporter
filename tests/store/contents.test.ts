...
// ---------------------------------------------------------------------------
// 入库范围：六类纪要 + 转写，录像与音频不入库
// ---------------------------------------------------------------------------

test('TEXT_GATEWAY_ASSET_TYPES 恰是六类纪要+转写，且是从引擎的 ALL_ASSET_KEYS 派生的（不是抄的短名）', () => {
...
test('buildAssetContent：录像/音频进来一律拒绝产出行——入库范围只有六类纪要+转写', async () => {
  await withFiles(async (dir) => {
