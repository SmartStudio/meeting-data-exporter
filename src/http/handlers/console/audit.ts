 ...
 * ## 一、时间量纲是 unix 秒，不是毫秒
 *
 * `src/store/audit.ts` 的注释多处写着「毫秒时间戳」，**与代码现实不符**：
 ...
 *
 * 按注释当成毫秒来算默认窗口的话，`now - 7 * 86400 * 1000` 会得到一个负数，
 * 下界形同虚设（全索引扫），而响应里还写着「最近 7 天」。本文件一律按秒处理，
 * 所有对外字段也是秒（与 `console/src/api/types.ts` 的口径一致）。
 * 不去改 store 的注释是因为那份文件属于 T3，并行任务不交叉改文件。
 *
 * ## 二、默认时间窗口必须看得见
 *
