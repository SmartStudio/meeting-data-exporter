 ...
 * host_user_id / start_time 等策略判定（`policy/access.ts` 的 `AccessGate.decide`）所需的会议属性——
 * 而这些属性只能来自两个列表接口（GET /v1/corp/records 按时间窗口、GET /v1/records 按
 * meeting_id / meeting_code），**二者都不支持按 meeting_record_id 反查**。
 *
